  ...
  def download_archive(self, save_path: str, split: Optional[str] = None) -> None:
    ...
    with zipfile.ZipFile(save_path, "a") as new_archive:
      with ThreadPoolExecutor(max_workers=self.num_workers):
        with tqdm(total=len(self.input_iterator), desc='Downloading Dataset'):
        ...
        #     progress.update()
            ...
            for input_ in tqdm(self.input_iterator, total=len(self.input_iterator), desc='Downloading Dataset'):
              self._write_archive(input_, new_archive, split)
